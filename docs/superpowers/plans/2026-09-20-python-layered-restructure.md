...
- [ ] **Step 0: Pin the page and the static assets with a real test, before touching the factory**

Nothing in the suite covers `template_folder` / `static_folder`. Every test
exercises `/api/*` JSON, so a wrong folder path leaves all tests green and the
app broken — Task 7 proved the wiring correct only with a one-off boot check
run outside pytest. You are about to rewrite `create_app`, which is exactly the
change that could break it. Write the regression test **first**, watch it pass
against the current factory, and only then start Step 1.

Add to a new `tests/web/test_app.py`:
...
Run it, confirm it passes, and record the new suite total. Then proceed to
Step 1 and keep it passing through the split.
